...
abstract class Captcha_Driver
{
	...
	public function __construct()
	{
		...
		$this->response = Session::instance()->get('captcha_response');
		if(!$this->response) $this->response = $this->generate_challenge();
...
//		Event::add('system.post_controller', array($this, 'update_response_session'));
	...
	{
		# Kohana::log("error", "checking $response against stored response " . Session::instance()->get('captcha_response'));
		$res = (strtoupper($response) === Session::instance()->get('captcha_response'));
	...
	public function image_render($html)
	{
		...
		if ($html)
		{
			return '<img alt="Captcha" src="'.url::site('captcha/'.Captcha::$config['group']).'?id=' . $rnd . '" width="'.Captcha::$config['width'].'" height="'.Captcha::$config['height'].'" />';
		}
